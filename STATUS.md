...
5. **Smoke runner + CI + coverage gaps — DONE (2026-05-24).** `scripts/__tests__/run-all.sh` (TAP summary, `--list`, self-skips live layers). Schema fixtures `review`/`touch`/`verify`. Direct smokes for `codex-safe.sh` (arg-validation, never invokes codex), `workflow-stash.sh` (throwaway-repo preservation), `cmux-cluster.sh` (early-guard paths: missing args / missing-infra / not-dispatch-ready, all exit before cmux), and `uds-pg-relay.mjs` (functional: echo-server → relay → UDS round-trip, no Postgres). CI: `.github/workflows/smoke.yml` runs `run-all.sh` on push/PR (Node 22, ubuntu). **14/14 smokes pass.**
