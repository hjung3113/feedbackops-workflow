...
5. **Smoke runner + CI + coverage gaps.** Schema fixtures for `review`/`touch`/`verify` — **DONE (2026-05-24)**. Still open: a `run-all` TAP runner; direct smokes for `codex-safe.sh`, `workflow-stash.sh` (in progress), `cmux-cluster.sh`, `uds-pg-relay.mjs`; optional GitHub Action.
