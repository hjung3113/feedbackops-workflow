---
name: agent-workflow
description: Run or adapt an evidence-gated multi-agent coding workflow with cmux, Claude, Codex, isolated worktrees, independent review, and host-side verification. Use whenever the user asks for /agent-workflow, parallel agent implementation, a Claude-Codex cluster, watched cmux workspaces, artifact-based handoffs, or installing this workflow into another repository. Do not use for a one-step read-only explanation.
...
1. **Scope** — resolve the issue/task, target repository, acceptance criteria, allowed touch set, and integration branch.
