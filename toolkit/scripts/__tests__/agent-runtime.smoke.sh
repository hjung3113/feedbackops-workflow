...
AGENT_WORKFLOW_RUNTIME_BIN="$BIN/codex" PATH="/usr/bin:/bin" bash "$RUNTIME" capabilities --runtime codex > "$TMP_DIR/pinned.json" 2>/dev/null; if [ $? -eq 0 ] && grep -F "\"executable\":\"$BIN/codex\"" "$TMP_DIR/pinned.json" >/dev/null; then ok 'generic absolute runtime pin is honored'; else bad 'generic runtime pin'; fi
