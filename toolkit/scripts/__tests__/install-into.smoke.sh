...
assert_exit "generic install with target instructions succeeds" PASS bash "$INSTALL" "$generic_agents"
assert_true "generic managed pointer resolves conductor persona" bash -c '
  ...
  [ -n "$ref" ] && [ -f "$1/$ref" ]
' _ "$generic_agents"
assert_exit "generic same-profile upgrade succeeds" PASS bash "$INSTALL" "$generic" --upgrade
assert_generic_profile "$generic"
