...
const groups=[{id:"test",required:true,commands:[{argv:["fake-test"],cwd:".",exit_code:0,duration_ms:5,output:"1 tests",output_truncated:false}],test_count:1}];
const verifyPass={schema_version:"1",artifact_type:"verify_result",producer_role:"VERIFIER",issue:188,branch:"main",head_sha:head,content_sha256:"aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa",cwd:worktree,verify_cmd:"smoke verify --filter shared-contract",target_profile:"smoke-profile",groups,verdict:{passed:1,failed:0,pending:0,exit_code:0},classifier:"PASS",failures:[],created_at:"2026-07-20T00:00:00Z"};
...
const fs=require("fs"); const path=require("path");
const [worktree,head]=process.argv.slice(2); const root=path.join(worktree,".review/evidence");
const verifyPass={schema_version:"1",artifact_type:"verify_result",producer_role:"VERIFIER",issue:188,branch:"main",head_sha:head,content_sha256:"aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa",cwd:worktree,verify_cmd:"smoke verify --filter shared-contract",target_profile:"smoke-profile",groups:[{id:"test",required:true,commands:[{argv:["fake-test"],cwd:".",exit_code:0,duration_ms:5,output:"1 tests",output_truncated:false}],test_count:1}],verdict:{passed:1,failed:0,pending:0,exit_code:0},classifier:"PASS",failures:[],created_at:"2026-07-20T00:01:00Z"};
...
const fs=require("fs"); const path=require("path");
const [worktree,head]=process.argv.slice(2); const root=path.join(worktree,".review/evidence");
const failedGroups=[{id:"test",required:true,commands:[{argv:["fake-test"],cwd:".",exit_code:1,duration_ms:5,output:"0 tests",output_truncated:false}],test_count:0}];
const failed={verify_cmd:"smoke verify --filter shared-contract",groups:failedGroups,verdict:{passed:0,failed:1,pending:0,exit_code:1},classifier:"FAIL",failures:[{code:"failed_tests",expected:"0",actual:"1"}],created_at:"2026-07-20T00:02:00Z"};
const passedGroups=[{id:"test",required:true,commands:[{argv:["fake-test"],cwd:".",exit_code:0,duration_ms:5,output:"1 tests",output_truncated:false}],test_count:1}];
const passed={verify_cmd:"smoke verify --filter unrelated",groups:passedGroups,verdict:{passed:1,failed:0,pending:0,exit_code:0},classifier:"PASS",failures:[],created_at:"2026-07-20T00:03:00Z"};
...
const forged={schema_version:"1",artifact_type:"verify_result",producer_role:"VERIFIER",issue:188,branch:"main",head_sha:head,content_sha256:"aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa",cwd:worktree,verify_cmd:"smoke verify --filter shared-contract",target_profile:"smoke-profile",groups:passedGroups,verdict:{passed:1,failed:0,pending:0,exit_code:0},classifier:"PASS",failures:[],created_at:passed.created_at,runs:[failed,passed]};
...
const fs=require("fs"); const path=require("path");
const [worktree,head]=process.argv.slice(2); const root=path.join(worktree,".review/evidence");
const matchingGroups=[{id:"test",required:true,commands:[{argv:["fake-test"],cwd:".",exit_code:0,duration_ms:5,output:"1 tests",output_truncated:false}],test_count:1}];
const laterGroups=[{id:"test",required:true,commands:[{argv:["fake-test","--scope=another"],cwd:".",exit_code:0,duration_ms:5,output:"2 tests",output_truncated:false}],test_count:2}];
const matching={verify_cmd:"smoke verify --filter shared-contract",groups:matchingGroups,verdict:{passed:1,failed:0,pending:0,exit_code:0},classifier:"PASS",failures:[],created_at:"2026-07-20T00:04:00Z"};
const later={verify_cmd:"smoke verify --filter another-scope",groups:laterGroups,verdict:{passed:2,failed:0,pending:0,exit_code:0},classifier:"PASS",failures:[],created_at:"2026-07-20T00:05:00Z"};
const aggregate={schema_version:"1",artifact_type:"verify_result",producer_role:"VERIFIER",issue:188,branch:"main",head_sha:head,content_sha256:"aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa",cwd:worktree,verify_cmd:later.verify_cmd,target_profile:"smoke-profile",groups:laterGroups,verdict:{passed:3,failed:0,pending:0,exit_code:0},classifier:"PASS",failures:[],created_at:later.created_at,runs:[matching,later]};
