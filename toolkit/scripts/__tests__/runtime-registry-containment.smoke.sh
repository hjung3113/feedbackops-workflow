...
# opencode) may live only inside lib/runtime-registry.cjs. Mirrors the
...
runtime_set_literal() {
  grep -E -q 'codex[[:space:]]*\|[[:space:]]*claude[[:space:]]*\|[[:space:]]*opencode' "$1" && return 0
  grep -E -q 'codex[[:space:]]+claude[[:space:]]+opencode' "$1" && return 0
  grep -E -q 'codex,[[:space:]]*claude,[[:space:]]*opencode' "$1" && return 0
  return 1
}
...
process.exit(
  same(RUNTIMES, ["codex", "claude", "opencode"])
  && STASH_BY.codex === "runtime" && STASH_BY.claude === "watchdog" && STASH_BY.opencode === "watchdog"
...
if grep -F -q -- '[--runtime codex|claude|opencode]' "$TMP_DIR/aw-usage.out"; then ok "AC-135-2 usage renders the runtime set from the registry byte-identically"; else not_ok "AC-135-2 usage renders the runtime set from the registry byte-identically"; fi
...
aw_refusal_ec=$?
if [ "$aw_refusal_ec" -eq 2 ] && grep -F -q -- 'ERROR: unknown_runtime: bogus (expected codex, claude, or opencode)' "$TMP_DIR/aw-refusal.out"; then ok "AC-135-2 unknown runtime refusal is byte-identical"; else not_ok "AC-135-2 unknown runtime refusal is byte-identical"; fi
...
if grep -F -q -- 'capabilities --runtime codex|claude|opencode' "$TMP_DIR/ar-usage.out"; then ok "AC-135-5 usage renders the runtime set from the registry byte-identically"; else not_ok "AC-135-5 usage renders the runtime set from the registry byte-identically"; fi
...
if grep -F -q -- '--runtime codex|claude|opencode' "$TMP_DIR/wd-usage.out"; then ok "AC-135-6 usage renders the runtime set from the registry byte-identically"; else not_ok "AC-135-6 usage renders the runtime set from the registry byte-identically"; fi
...
printf '%s\n' 'case "$RUNTIME" in codex|claude|opencode) ;; *) exit 2 ;; esac' >> "$pipe_fixture"
...
printf '%s\n' 'for runtime in codex claude opencode; do :; done' >> "$space_fixture"
...
printf '%s\n' 'echo "expected codex, claude, opencode"' >> "$comma_fixture"
