...
if (mode === "acquire-lock") {
  ...
  try {
    publishViaPendingDir(lockDir, pending => {
      writeAtomicJson(path.join(pending, path.basename(lockFile)), { version: 1, issue: String(issue), admission_key: key, pid: Number(ownerPid) || process.ppid, status: "locked" });
    });
    process.exit(0);
  } catch (_) {
    process.exit(1);
...
if (mode === "publish") {
  ...
  try {
    ...
    publishViaPendingDir(target, pending => {
      writeAtomicJson(path.join(pending, txName), tx);
      let exists = false;
      try { fs.lstatSync(target); exists = true; }
      catch (error) { if (!error || error.code !== "ENOENT") throw error; }
      if (exists) throw new Error("admission_target_exists");
    });
  } catch (_) {
    process.exit(1);
  }
...
if (mode === "commit-admission") {
  ...
  const tx = readJsonOrNull(path.join(integratedDir, txName));
...
if (mode === "recover-lock") {
  ...
  const owner = readJsonOrNull(lockFile);
  ...
  if (!owner) {
    ...
    if (!quarantineThenDelete(path.dirname(lockFile))) process.exit(2);
    process.exit(0);
  }
  if (processAlive(owner.pid)) process.exit(1);
  const state = readJsonOrNull(root);
  ...
  const singletonTx = readJsonOrNull(path.join(singleton, txName));
...
if (mode === "commit") {
  try {
    const file = path.join(integratedDir, txName); const tx = readJsonOrNull(file);
    if (!tx) process.exit(2); tx.status = "committed"; writeAtomicJson(file, tx);
    const marker = path.join(admissionDir, txName); if (fs.existsSync(marker)) writeAtomicJson(marker, tx);
...
const state = readJsonOrNull(roundState);
...
if (fs.existsSync(admissionDir)) {
  const normal = readJsonOrNull(path.join(admissionDir, txName));
  if (normal && normal.kind === "normal" && safeIssue(issue) && safeKey(normal.admission_key)
    ...
    if (advanced) {
      if (normal.status !== "committed") {
        try { normal.status = "committed"; writeAtomicJson(path.join(admissionDir, txName), normal); } catch (_) { process.exit(2); }
      }
...
if (expectedRouteDigest) {
  ...
  const ordinal = readJsonOrNull(ordinalFile);
...
if (advanced) {
  try { tx.status = "committed"; writeAtomicJson(path.join(integratedDir, txName), tx); if (fs.existsSync(path.join(admissionDir, txName))) writeAtomicJson(path.join(admissionDir, txName), tx); } catch (_) { process.exit(2); }
  process.exit(0);
}
