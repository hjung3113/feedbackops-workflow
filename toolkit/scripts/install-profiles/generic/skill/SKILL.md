...
Resolve the target repository and its installed `.agent-workflow` directory.
Read the target instructions and the installed generic playbook before work.
Use `scripts/agent-workflow.sh dispatch` with explicit, independently selected
runtime (`codex|claude|opencode`), role (including conductor), and available
orchestrator. Run `scripts/agent-workflow.sh capabilities` before admission;
...
verification separate. Verify through `scripts/target-verify.sh` with a
