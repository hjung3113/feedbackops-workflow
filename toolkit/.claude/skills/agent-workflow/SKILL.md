...
- All write-capable runtime work runs through `$WF/scripts/agent-workflow.sh dispatch`, after explicit runtime, role, and `cmux|orca|herdr` transport selection. It reaches the shared dispatch core and typed runtime adapter; do not hand-build a transport/runtime command. Probe the requested runtime/role before admission; unavailable capability has a machine-readable failure and never falls back. Herdr additionally requires an inherited session (`HERDR_ENV=1` and non-empty `HERDR_SOCKET_PATH`); its returned workspace ID is the external handle, while workspace liveness and transport receipts are not completion evidence. Herdr receipts record launch intent/provenance, not confirmed command delivery. OpenCode additionally requires an explicit deny-first permission configuration (`permission["*"]:"deny"`; write only adds `permission.edit:"allow"`).
