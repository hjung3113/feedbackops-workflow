...
- All write-capable runtime work runs through `$WF/scripts/agent-workflow.sh dispatch`, after explicit runtime, role, and `cmux|orca|herdr` transport selection. It reaches the shared dispatch core and typed runtime adapter; do not hand-build a transport/runtime command. Probe the requested runtime/role before admission; unavailable capability has a machine-readable failure and never falls back. Herdr additionally requires an inherited session (`HERDR_ENV=1` and non-empty `HERDR_WORKSPACE_ID`/`HERDR_TAB_ID`/`HERDR_PANE_ID`); its returned workspace ID is the external handle, while workspace liveness and transport receipts are not completion evidence. Herdr receipts record launch intent/provenance, not confirmed command delivery. OpenCode additionally requires an explicit deny-first permission configuration (`permission["*"]:"deny"`; write only adds `permission.edit:"allow"`).
- `--execution-mode headless|live-tui` is a CLI-only axis separate from permission mode; `headless` remains the default, and phase-1 `live-tui` admission is limited to implementation/write seats with a complete adapter live-session contract. Live `LIVE.json` and v4 receipts are launch/liveness evidence, never completion evidence, and missing live capability refuses without a headless fallback. Codex live launches additionally mutate the user's codex config: at the real launch (via the runtime's `pre-launch` seam — launch-spec emission stays side-effect-free), codex merges a `[projects."<worktree>"] trust_level = "trusted"` entry into `${CODEX_HOME:-$HOME/.codex}/config.toml` (TOML-safe, mode-preserving, fails closed on unreadable config) so codex's first-run trust prompt never blocks the session (#215).
...
- Parallel eligibility requires a canonical execution plan and `lib/parallel-plan.cjs decide`; absent, broad, dynamic, overlapping, dependency-bound, shared-mutation, or isolation/budget-unproven seats serialize. Worker claims and transport state never establish independence.
...
8. **Verify** — run `$WF/scripts/target-verify.sh <target-profile.json> <issue>` outside the Codex sandbox and require its current HEAD-bound canonical VERIFY. Same-identity reruns aggregate, so any failed run keeps readiness red until a new HEAD.
...
Optional telemetry is never implicit. Collect it only when the user/operator opts in, using `$WF/scripts/telemetry.sh` against canonical artifacts. A green sample requires matching canonical CLOSURE, INTEGRATION, and CANDIDATE-EVIDENCE sources with current byte digests and generation/RUN freshness, not REVIEW/VERIFY pass alone. A policy-routed sample also needs the canonical v3 or v4 transport receipt; the collector accepts it only when its complete routing tuple matches the host admission binding, then derives the tuple and salted route pseudonym from that host-bound record, never from telemetry CLI values. Salt and store paths must resolve inside the target. Treat reports as advisory evidence; never feed them directly into automatic model allocation or tier mutation, and never compare a mixed-model retry chain as if its first model owned every attempt.
