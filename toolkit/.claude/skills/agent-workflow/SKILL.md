...
- `--execution-mode headless|live-tui` is a CLI-only axis separate from permission mode; `headless` remains the default, and phase-1 `live-tui` admission is limited to implementation/write seats with a complete adapter live-session contract. Live `LIVE.json` and v4 receipts are launch/liveness evidence, never completion evidence, and missing live capability refuses without a headless fallback. Codex live launches additionally mutate the user's codex config: while emitting the launch spec, the codex runtime merges a `[projects."<worktree>"] trust_level = "trusted"` entry into `${CODEX_HOME:-$HOME/.codex}/config.toml` (TOML-safe, mode-preserving, fails closed on unreadable config) so codex's first-run trust prompt never blocks the session (#215).
