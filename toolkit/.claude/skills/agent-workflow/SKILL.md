...
Optional telemetry is never implicit. Collect it only when the user/operator opts in, using `$WF/scripts/telemetry.sh` against canonical artifacts. A green sample requires matching canonical CLOSURE, INTEGRATION, and CANDIDATE-EVIDENCE sources with current byte digests and generation/RUN freshness, not REVIEW/VERIFY pass alone. A policy-routed sample also needs the canonical v3 transport receipt; the collector derives its tuple and salted route pseudonym only after host admission-binding validation, never from CLI routing values. Salt and store paths must resolve inside the target. Treat reports as advisory evidence; never feed them directly into automatic model allocation or tier mutation, and never compare a mixed-model retry chain as if its first model owned every attempt.
