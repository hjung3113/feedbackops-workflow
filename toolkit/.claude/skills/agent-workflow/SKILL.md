...
6. **Control redispatch** — after every failed implementation round, preserve schema/issue/HEAD-bound VERIFY/REVIEW evidence and classify exactly one primary origin in `round_control.failures[]`; closing a failure requires canonical passing `closed_by` evidence and closed history remains before the active open cycle. A write-capable same-issue redispatch must pass `--round-state <state> --manifest-revision <revision>` to `cmux-dispatch.sh`; it atomically records pre-cmux intent, binds admission to the CLI issue/worktree, and consumes the immutable issue/ordinal/mode key in the Git common dir. Dispatch only that mode. A diagnosis gate rechecks oracle/contract before one integrated fix batch; watchdog retries and model escalation do not clear it.
