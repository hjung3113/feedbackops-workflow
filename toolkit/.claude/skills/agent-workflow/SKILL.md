...
6. **Control redispatch** — after every failed implementation round, preserve its evidence and classify exactly one primary origin in `round_control.failures[]`, then run `$WF/scripts/redispatch-check.sh --round-state <state> --manifest-revision <revision>`. Dispatch only the emitted mode. A diagnosis gate rechecks oracle/contract before one integrated fix batch; watchdog retries and model escalation do not clear it.
