...
4. **Dispatch** — write `.review/ISSUE-<N>-PROMPT.txt`, choose either explicit playbook-selected `--model`/`--effort` pins or `--allocate --allocator-role implementation`, and include `--tier`, then call `cmux-dispatch.sh`. It atomically records the fully quoted watchdog argv in a launch-unique worktree-local runner and gives cmux only a short relative runner command; do not recreate an issue-wide runner or inline command. Preserve its direct exit code and accept only RUN/BLOCKER identity fresh for that launch; follow the playbook's dispatch liveness operator rules instead of hand-rolling a file-only poller. Standard/Full Cluster initial writes and every redispatch pass `--round-state <state> --manifest-revision <revision>`; initial admission validates schema, active lifecycle, issue, tier, revision, worktree, live HEAD, and base freshness before cmux starts. Standard state keeps `pr_draft` and `review` pointers. Trivial initial writes pass `--tier trivial` without ROUND-STATE, preserving the `pr_draft`-only tax boundary. Use the liveness-only `--read-only` for non-review thinking seats. REVIEWER uses `--produce-review`, which enforces a read-only Codex sandbox and publishes only a schema/issue/live-HEAD-validated canonical REVIEW. Tune liveness budgets only when needed.
...
8. **Verify** — run the target's verifier outside the Codex sandbox. If the target matches the bundled backend-pnpm/Vitest adapter, configure its target-owned `VERIFY_CLEAN_COMMAND`, use no-filter `verify.sh` for the full backend module by default, and require a current `ISSUE-<N>-VERIFY.json` containing clean-state evidence. Narrow filters require an explicit touched-behavior coverage check; same-identity reruns are one `runs[]` aggregate, so any failed run keeps canonical readiness red until a new HEAD. Otherwise use the target-native test gate and clearly report that the bundled canonical verifier adapter is not applicable; never fabricate its artifact.
