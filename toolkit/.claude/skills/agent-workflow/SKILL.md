...
- All **write-capable Codex implementation** runs through `$WF/scripts/agent-workflow.sh dispatch`, after the user explicitly selects `cmux` or `orca`. It reaches the shared dispatch core and `codex-safe.sh`; do not hand-build a transport command or call `codex exec` directly for writes. Historical callers may keep using `cmux-dispatch.sh`, which explicitly selects cmux through the same core.
...
1. **Scope** — resolve the issue/task, target repository, acceptance criteria, allowed touch set, canonical structured prohibitions, and integration branch. Before locking scope for an exported-contract change, follow the playbook's pre-scope-lock impact pass and record its compile-atomic boundary plus convention watches in ROUND-STATE; for repository-dependent Full Cluster ARCH decisions and test-matrix rows, apply the feasibility-evidence and test-matrix contracts. Before the first Standard/Full Cluster write, CONDUCTOR generates the complete canonical `ISSUE-<n>-ROUND-STATE.json`, including `contract.prohibitions[]`; prompt regexes are not authority. When more than one write seat is proposed, CONDUCTOR also creates canonical `ISSUE-<n>-EXECUTION-PLAN.json`; a one-seat plan is the sequential compatibility form. Escalation revises the same ROUND-STATE and plan binding. Trivial remains pr_draft-only. Prompt amendments cannot override canonical state.
...
4. **Dispatch** — for Standard/Full and redispatch, first create uncommitted `.review/ISSUE-<N>-CONTEXT.md`, ask one user-facing reverse-question batch when needed, then compress `.review/ISSUE-<N>-PROMPT.md`; keep authoring context out of CODEX. Its delimited JSON AC block must exactly copy canonical ROUND-STATE criteria or the shared core rejects the launch before side effects. Make the transport choice explicit with `--orchestrator cmux|orca`, `AGENT_WORKFLOW_ORCHESTRATOR`, or target-local `.agent-workflow/workflow-config.json`; there is no default or fallback. Choose either explicit playbook-selected `--model`/`--effort` pins or `--allocate --allocator-role implementation`, include `--tier`, then call `agent-workflow.sh dispatch`. The core resolves the caller's Codex executable to an absolute host path before admission and pins it through the runner; target config cannot inject one. A planned write additionally passes canonical `--execution-plan <plan> --seat <id>`; admission binds issue, revision, base HEAD, worktree, seat, and exact ROUND-STATE allowlist before consuming an immutable same-seat admission. The core atomically records the fully quoted watchdog argv in a launch-unique worktree-local runner and gives the selected adapter only the exact worktree, seat name, and relative runner; do not recreate an issue-wide runner or inline command. Preserve its direct exit code and accept RUN/BLOCKER only when their `mtime + started_at` identity is fresh for that launch; use the playbook's dispatch-liveness operator rules rather than a file-only poller. Standard/Full Cluster initial writes and every redispatch pass `--round-state <state> --manifest-revision <revision>`; Trivial initial writes remain `pr_draft`-only. Use `--read-only` for non-review thinking seats and `--produce-review` for canonical read-only REVIEW publication.
