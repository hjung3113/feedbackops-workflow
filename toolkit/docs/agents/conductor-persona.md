...
- **Repeated-round admission** — classify every failed implementation round in `round_control.failures[]` with exactly one primary origin, schema/issue/HEAD-bound VERIFY/REVIEW evidence, owner, and typed next action; closing one requires canonical passing `closed_by` evidence and closed history remains a prefix before the active open cycle. Pass canonical state/revision to `cmux-dispatch.sh`: it atomically records write intent, binds admission to the CLI issue/worktree, and consumes the immutable issue/ordinal/mode key in the Git common dir. Two consecutive equal active origins or a proposed third redispatch enters diagnosis: recheck oracle/contract first, ground the decision in one hard fact and a passing analog, then permit at most one manifest revision and one integrated fix batch. Security may stop earlier; model escalation and watchdog attempts never reset the circuit.
