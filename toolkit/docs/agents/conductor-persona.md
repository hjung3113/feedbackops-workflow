...
- **Transport selection:** select `cmux`, `orca`, or `herdr` explicitly through CLI > environment > target config. Herdr requires an inherited session (`HERDR_ENV=1` and non-empty `HERDR_WORKSPACE_ID`/`HERDR_TAB_ID`/`HERDR_PANE_ID`); missing capability or session context fails closed and never falls back.
