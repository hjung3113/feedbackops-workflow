...
- `ISSUE-N-ROUND-STATE.json` — CONDUCTOR-owned canonical contract state. It replaces prose amendments and carries the current contract, AC manifest view (including `acceptance.expected_test_count`), optional compile-atomic `contract.chunk_boundary`, optional repeated-failure `round_control`, locked/open decisions, commit range, freshness provenance, live probes, and artifact pointers. Failure entries bind their preserved evidence to a content hash and observed HEAD; closing one also requires verifier/review closure evidence. Watchdog attempts are not failure rounds. Its top-level `revision` is the `manifest_revision` pinned by every dispatch; CODEX never edits it.
...
- `<git-common-dir>/agent-workflow/redispatch-admissions/issue-N-dispatch-O-mode` — atomic runtime consumption marker created by `cmux-dispatch.sh` after the read-only circuit gate passes. Its immutable issue/ordinal/mode identity survives revision edits, failure-ID renames, and worktree recreation, but it is not contract authority and must not replace ROUND-STATE evidence.
