...
- `ISSUE-N-PR-DRAFT.json` — CODEX → REVIEWER implementation handoff (commit SHA, files, claimed tests, risks). Its optional `verify_result` field is deprecated and ignored; CODEX cannot verify its own work. Re-review capsule generation requires `worktree_path` to resolve to the canonical ROUND worktree and `base_sha` to equal ROUND-STATE; `base_branch` identifies the integration base for other freshness checks.
- `ISSUE-N-BLOCKER.json` — CODEX abort report (no commit, why stopped). It records the producer-observed `head_sha` plus structured cause: `reason_code` (enum) + `blocking_fact` (concrete observed fact naming the ACTUAL files/symbols hit — never copied from the dispatch prompt) + `attempted_commands` (exact commands run before aborting) + `needed_decision` (the specific human/ARCHITECT call needed to unblock). `recommended_actions` has been REMOVED — free prose was the affordance that caused prompt-template leakage; with `additionalProperties:false`, any artifact still carrying it is now REJECTED. Only `active` and `final` BLOCKER artifacts are consumable; `superseded` is ignored and redispatch admission returns `superseded_evidence_artifact`. Legacy BLOCKER files without `head_sha` remain historical display evidence only; regenerate a current artifact at the observed commit before using one for redispatch admission.
- `ISSUE-N-REVIEW.json` — REVIEWER findings + patch_instructions for ARCHITECT. The sanctioned `agent-workflow.sh dispatch --produce-review` path runs the reviewer in an actual read-only Codex sandbox, captures its final JSON outside that sandbox, validates schema/producer/issue/live HEAD, and atomically publishes this canonical file. Invalid output, a non-zero reviewer exit, pane prose, and CONDUCTOR transcription never replace it.
...
- `<git-common-dir>/agent-workflow/redispatch-admissions/issue-N-dispatch-O` — atomic per-ordinal runtime consumption marker created by the shared dispatch core; mode changes cannot replay it. Integrated fix also creates `issue-N-integrated-fix`, an issue-wide singleton. These survive state edits and worktree recreation but are not contract authority and must not replace ROUND-STATE evidence.
