...
- `ISSUE-N-REVIEW.json` — independent REVIEWER findings + patch instructions. The sanctioned `agent-workflow.sh dispatch --produce-review` path uses the selected capability-probed runtime in its required read-only mode, validates schema/producer/issue/live HEAD, and atomically publishes this canonical file. Invalid output, a non-zero reviewer exit, pane prose, and CONDUCTOR transcription never replace it.
