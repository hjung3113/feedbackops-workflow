...
- `.review/.redispatch-admission-*` — atomic runtime consumption markers created by `cmux-dispatch.sh` after the read-only circuit gate passes. They prevent reuse of the same ROUND-STATE admission but are not contract authority and must not replace ROUND-STATE evidence.
