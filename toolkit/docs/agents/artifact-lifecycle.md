...
- `.review/.redispatch-admission-*` — atomic runtime consumption markers created by `cmux-dispatch.sh` after the read-only circuit gate passes. Their failure-set/ordinal/mode identity is stable across revision-only edits and prevents reuse of the same ROUND-STATE admission, but they are not contract authority and must not replace ROUND-STATE evidence.
