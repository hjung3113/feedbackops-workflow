...
# For codex launches the first-run trust prompt is additionally prevented upstream (#215): the codex runtime member pre-seeds codex's own per-directory trust store while emitting the launch spec — `codex_trust_preseed` in `scripts/lib/codex-policy.sh` merges a `[projects."<resolved-cwd>"] trust_level = "trusted"` entry into `$CODEX_HOME/config.toml` (default `~/.codex`) before any transport launches codex — so every live transport benefits, transport files never branch on a runtime name, and codex's trust screen never renders. The merge is TOML-safe (whitespace-normalized table matching, in-table key insertion, existing file mode preserved, fresh files 0600, lock-serialized with unique temp files and dead-owner stale-lock reclaim), and any read failure other than ENOENT fails closed instead of rewriting an unreadable config. This is a user-visible mutation: selecting live-tui codex dispatch writes the worktree's trust entry into the user's codex config. Other runtimes never touch codex config.
