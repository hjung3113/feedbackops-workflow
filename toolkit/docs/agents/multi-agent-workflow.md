...
- **`--for-paths "<paths>"` narrows the suite to covering smokes (#165).** `scripts/__tests__/smoke-coverage.manifest` maps repo-relative source paths to the smokes that source/exec/copy them (`<source-path> <smoke-basename>` per line). Each given path (space- or newline-separated) matches manifest sources exactly or by string prefix; the union of matched smokes runs. If any given path has no manifest coverage, the runner prints `WARNING: ... falling back to the full suite` to stderr and runs the full inventory — an incomplete mapping must fail open, never silently skip a real smoke. Update the manifest mechanically (grep new smokes for the production paths they reference) whenever a smoke's covered sources change; a smoke with no production-path references simply stays out of the manifest and any path mapping to it falls back safely. `--for-paths` and `--list` together is a usage error (exit 2), as is an empty value.
- **Known flakes are registered, not re-diagnosed (#165).** `scripts/__tests__/flake-registry.manifest` holds one `<smoke-basename> <owner-github-username> <expiry-YYYY-MM-DD> <reason-slug>` line per known flake. A failing smoke with a well-formed, unexpired registration prints `FLAKY - <name> (exit N, known flake: ...)`, still captures/redacts/retains its diagnostic, but does not count against the exit code (exit 0 iff `PASSED + FLAKY == TOTAL`). Expired registrations auto-graduate back to ordinary failures; malformed lines warn on stderr and are treated as absent. The summary line stays `--- N/N passed` when no flakes fired and appends `($FLAKY known-flake)` only when one or more did. Registering a flake is an owner decision with an explicit expiry, not a way to silence a real regression.
