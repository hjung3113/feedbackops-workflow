...
Every write launch atomically creates a pre-cmux attempt marker, so a crash before RUN/BLOCKER creation cannot make the next launch look initial. When that marker, a same-issue RUN/BLOCKER, or canonical failure history exists, `cmux-dispatch.sh` refuses write dispatch without state/revision inputs. It executes `redispatch-check.sh` immediately before cmux creation, requires at least one classified active failure, binds the result's issue and real worktree to the CLI target, and atomically creates a failure-set/ordinal/mode-specific admission directory. This identity deliberately excludes mutable revision, so a revision-only edit cannot replay the same batch. A crash after consumption requires investigation and a new classified failure or explicit blocker rather than deleting the marker. `--dry-run` evaluates but never consumes admission. Read-only seats remain outside the implementation circuit.

Each immutable failure entry has one `primary_origin`—`environment`, `dispatch_contract`, `implementation`, `test_oracle`, `verification_harness`, or `integration_drift`—plus optional unique `secondary_origins[]`, failed AC ids, an owner, a typed next action, and evidence pointers bound to content hash and observed HEAD. Primary origin alone drives the breaker; secondary origins preserve nuance without diffusing ownership. Model or effort escalation is not an action kind and never clears a failure. Preserve failed VERIFY/REVIEW evidence at the referenced history path before a later run replaces a current artifact. The gate resolves each evidence file inside the declared worktree, verifies its SHA-256 and repository commit, validates VERIFY/REVIEW artifacts against their schemas, binds their issue and artifact HEAD to the reference, and requires at least one genuinely failing verdict per failure. It also rejects a ROUND-STATE whose `head_sha` differs from the live worktree. Failures with AC ids retain those ids per entry. Changing a failure to `closed` additionally requires a passing, hash/HEAD-bound `closed_by` VERIFY/REVIEW artifact, so editing status or relabeling arbitrary text cannot reset the active sequence.
