...
# For codex launches the first-run trust prompt is additionally prevented upstream (#215): `launch-live` merges a `[projects."<resolved-cwd>"] trust_level = "trusted"` entry into codex's own `$CODEX_HOME/config.toml` (default `~/.codex`) before `agent start`, TOML-safely preserving all other config, so codex's trust screen never renders and live capability does not depend on `blocked` detection for that case. Other runtimes are forwarded as data and never touch codex config.
