...
- Added opt-in local append-only model/task telemetry with salted project pseudonyms, canonical artifact digests, truthful observed/estimated/unavailable usage, concurrency-safe idempotence, and explicit single-sample deletion. Green consumes the parallel-safety producer's canonical closure plus its canonical integration/evidence sources through byte-identical shared #14 schemas, actual byte digests, strict semantic RFC3339 dates, and generation/RUN freshness ordering; salt/store realpaths must remain target-local. Policy-routed telemetry v2 validates a v3 transport receipt against the current host admission binding, derives runtime/model/effort/transport, and stores only a local-salt HMAC route pseudonym; raw digest and CLI tuple injection are refused. Retry reports enforce immutable project/issue/round/revision lineage, contiguous admitted attempts and valid edges, expose per-attempt allocation, and suppress mixed-model chains from single-model cohorts. v2 routing cohorts use homogeneous policy samples and complete independent-chain thresholds with a confounder warning. Reports are advisory and cannot mutate allocation or tier policy.
