...
gate는 선언된 worktree의 live HEAD와 증거 파일의 실제 hash를 검증합니다. VERIFY/REVIEW 포인터는 artifact schema, issue, observed HEAD와 모순 없는 실패 verdict까지 대조하고 origin별 fix action도 고정합니다. closure는 정확한 실패 AC 집합과 canonical verify filter 또는 REVIEW checklist item을 닫아야 하며, REVIEW closure는 `lifecycle:"final"`, `status:"pass"`, 모든 checklist의 `met:true`까지 동시에 만족해야 합니다. 이 셋 중 하나가 틀리면 exit 2의 기존 `failure_closure_not_verified` 코드와 함께 `review_lifecycle_not_final`, `review_status_not_pass`, `review_checklist_unmet` 중 하나를 `errors[0].detail`로 반환합니다. PASS는 실패 증거보다 뒤이고 live HEAD 계보 안에 있는 동일 브랜치만 인정합니다. closed history는 active open cycle 앞의 prefix여야 합니다. 실제 write redispatch는 `cmux-dispatch.sh`에도 같은 `--round-state`와 `--manifest-revision`을 전달해야 합니다. dispatch 직전에 CLI issue/worktree와 admission identity를 다시 대조하고 immutable issue/ordinal admission을 Git common dir에서 원자적으로 한 번 소비합니다. integrated fix는 이슈 전체 singleton도 소비하므로 mode·revision·failure ID 변경이나 worktree 재생성으로 재사용할 수 없습니다. 모든 write launch는 cmux 전에 attempt marker를 원자적으로 획득해 pre-RUN 실패와 동시 최초 실행도 제어합니다. `--dry-run`은 검사만 하고 소비하지 않으며 `--read-only` 좌석은 구현 회로 밖입니다.
gate는 선언된 worktree의 live HEAD와 증거 파일의 실제 hash를 검증합니다. VERIFY/REVIEW 포인터는 artifact schema, issue, observed HEAD와 모순 없는 실패 verdict까지 대조하고 origin별 fix action도 고정합니다. 예외적으로 `dispatch_contract` + `contract_fix` 라운드는 schema·issue·hash가 맞고 evidence의 referenced HEAD가 실제 commit이면서 BLOCKER가 기록한 producer `head_sha`와 같은 canonical BLOCKER 하나만으로 재디스패치 증거가 될 수 있습니다. BLOCKER lifecycle은 `active`와 `final`만 소비하며 `superseded`는 안정적인 `superseded_evidence_artifact` 오류로 admission 전에 거부합니다. 이는 BLOCKER를 일반 VERIFY/REVIEW 대체물로 만드는 규칙이 아니며 다른 origin은 기존 실패 VERIFY/REVIEW를 계속 요구합니다. `head_sha`가 없는 구형 BLOCKER는 표시용 이력으로만 남기고 admission에는 현재 관측 commit에서 재생성해야 합니다. closure는 정확한 실패 AC 집합과 canonical verify filter 또는 REVIEW checklist item을 닫아야 하며, 실패 증거보다 뒤이고 live HEAD 계보 안에 있는 동일 브랜치의 PASS만 인정합니다. closed history는 active open cycle 앞의 prefix여야 합니다. 실제 write redispatch는 `cmux-dispatch.sh`에도 같은 `--round-state`와 `--manifest-revision`을 전달해야 합니다. dispatch 직전에 CLI issue/worktree와 admission identity를 다시 대조하고 immutable issue/ordinal admission을 Git common dir에서 원자적으로 한 번 소비합니다. integrated fix는 이슈 전체 singleton도 소비하므로 mode·revision·failure ID 변경이나 worktree 재생성으로 재사용할 수 없습니다. 모든 write launch는 cmux 전에 attempt marker를 원자적으로 획득해 pre-RUN 실패와 동시 최초 실행도 제어합니다. `--dry-run`은 검사만 하고 소비하지 않으며 `--read-only` 좌석은 구현 회로 밖입니다.
...
검증 실패는 기존 사람용 `FAIL:` 메시지와 함께 원인별 `VERIFY_FAILURE_JSON=` 줄을 내보냅니다. canonical VERIFY artifact의 `failures[]`, `clean_state.{sentinel,migration_hash}`, `clean_state.role`도 같은 검증 증거를 보존합니다. 같은 issue/branch/HEAD/cwd/DB에서 좁은 filter를 여러 번 실행하면 하나의 artifact `runs[]`에 순서대로 보존되며, 하나라도 FAIL이면 이후 로컬 PASS도 aggregate를 녹색으로 바꾸지 못하고 nonzero로 끝납니다. 새 HEAD는 새 aggregate를 시작하고, `runs`가 없는 이전 flat artifact만 한 번의 legacy run으로 해석됩니다. publication은 같은 디렉터리 임시 파일을 schema/semantic 검증한 뒤 원자적으로 교체하므로 실패한 실행은 기존 red evidence를 덮어쓰지 않습니다. 입력의 추가 필드는 거부되고 artifact에는 선언된 필드만 투영됩니다. 이 값에는 URL, 비밀번호, 고객 데이터 등 비밀을 넣지 않습니다. 이름과 무관하게 probe가 확인한 superuser role은 fail-closed입니다.
...
| `ISSUE-N-VERIFY.json` | 현재 HEAD에 대한 VERIFIER의 canonical 검증 증거; 같은 identity의 여러 filter run은 `runs[]`에 집계 |
