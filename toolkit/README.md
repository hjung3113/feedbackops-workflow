...
설정값은 transport `cmux|orca|herdr`, runtime `codex|claude|opencode`, role `conductor|architect|implementation|reviewer|verifier|visual|release`만 허용하며, 위 우선순위에 따라 언제든 명시적으로 교체할 수 있습니다. `capabilities`로 선택 전에 세 adapter와 세 runtime을 함께 점검하고, dispatch receipt는 실제 선택된 transport·runtime·role을 기록합니다.
