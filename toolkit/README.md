...
분리된 워크트리와 독립적인 검증으로 cmux·Orca·Herdr × Codex·Claude Code·OpenCode 작업을 운영하는 **opt-in 멀티 에이전트 워크플로 툴킷**입니다. 병렬 작성자의 작업을 디스크 산출물로 추적하고, 에이전트의 “완료했습니다”가 아니라 현재 HEAD에 결속된 증거로 병합 여부를 판단합니다.
...
Standard/Full Cluster 최초 write 전에는 CONDUCTOR가 `schemas/round_state.schema.json` 전체를 만족하는 `ISSUE-123-ROUND-STATE.json`을 만들고 issue, tier, revision, 실제 worktree, live HEAD, base freshness에 결속해야 합니다. 초기 scaffold는 `scripts/round-state-init.sh <issue-number> --worktree <path>`가 기계적 필드(issue 제목·AC 후보는 `gh issue view`에서, base/head SHA·worktree 경로는 git에서)를 채우고 schema 검증 후 생성합니다. tier, objective, touch_allowlist, prohibitions, verify_filter, test_discovery_command, expected_test_count은 TODO placeholder로 남고 저자가 직접 결정해야 하며, 이전 issue 파일 복사는 더 이상 필요하지 않습니다. Standard는 별도 mini-state를 만들지 않고 `pr_draft`와 `review` pointer를 유지합니다. Trivial 최초 write만 `--tier trivial`과 기존 `pr_draft`-only 계약을 사용합니다.

CONDUCTOR는 dispatch 전에 `.review/ISSUE-123-BRIEF.md`에 원자료를 정제 없이 모으고, 필요한 사용자 역질문을 한 번(최대 4문항)으로 끝낸 뒤, `.review/ISSUE-123-PROMPT.md`를 압축합니다. Standard/Full과 canonical redispatch는 prompt 안의 delimited JSON AC block이 ROUND-STATE `acceptance.criteria[]`의 ID·statement·순서를 정확히 복사하지 않으면 launch 전에 거부됩니다. AC block은 `scripts/round-state-render-ac.sh --round-state <path>` 출력을 prompt에 붙여 단일 출처에서 만들 수 있습니다. 두 Markdown 파일은 uncommitted/non-archival scratch이며, `model-alloc.json`의 `prompt_authoring.target_tokens`는 길이 안내·telemetry일 뿐 launch 거부 조건이 아닙니다.
...
실행 방식은 권한 `--mode`와 별도의 `--execution-mode headless|live-tui` 축입니다. 기본값은 `headless`이며(단, `orca` adapter는 명시적 `--execution-mode` 없이도 기본값이 `live-tui`입니다) 기존 runner/watchdog 경로를 그대로 사용합니다. T1의 `live-tui`는 CLI에서만 선택할 수 있고 implementation/write seat만 허용합니다. runtime이 shell-free `launch-spec` JSON(`argv[]`, `env`, `prompt_delivery`)을 만들고 adapter가 live session을 제어합니다. 준비 완료와 prompt 시작 activity는 `.review/ISSUE-N-launch.*/LIVE.json` 및 schema v4 transport receipt에 기록되지만 completion은 아니며, canonical PR-DRAFT/BLOCKER/REVIEW/VERIFY가 계속 authority입니다. 필요한 live capability가 증명되지 않으면 headless로 바꾸지 않고 거부합니다. codex live 실행은 실제 launch 직전(pre-launch 시점, launch-spec preflight는 무부효과)에 codex 자체 trust store인 `${CODEX_HOME:-$HOME/.codex}/config.toml`에 해당 worktree 경로의 `[projects."<경로>"] trust_level = "trusted"` 항목을 TOML-safe하게 병합합니다(기존 설정과 파일 권한은 보존, ENOENT 외 읽기 실패는 거부). 이는 사용자 codex 설정에 대한 명시적인 변경이며, 첫 실행 trust 프롬프트(#212)가 애초에 뜨지 않게 합니다. Herdr의 live 경로는 `pane run`이 아니라 `agent start/prompt --wait/wait/read/send-keys/get` facade를 사용하고, fresh/untrusted worktree에서 production과 같은 trust-prompt characterization을 수행합니다. Herdr 0.8.0은 `agent start`의 구조화된 `timeout` 뒤 `agent get`에서 `agent_not_found`를 반환하므로 live capability는 계속 unavailable이고 headless는 그대로입니다. `agent_not_ready` 뒤 retained `agent_status:"blocked"`는 v0.8.2+에서 별도 검증할 미래 계약이며 #212에서 admit하지 않습니다. false-ready, malformed, mismatched 또는 non-blocked 결과도 fail-closed입니다.
...
cmux live(T4)는 `claude-teams`/`codex-teams`/`omo`가 아니라 generic direct-argv `cmux run ... -- <argv...>` 경로만 사용합니다. live token은 설치된 바이너리의 `--help`로 증명하며(버전 숫자가 아님), run envelope의 `workspace`·`surface`·`terminal_id`를 구조화 handle로 사용해 `external_handle` alias를 만들지 않습니다. prompt는 base64 `--bytes`로 byte-exact 전달 후 별도 `send-key enter`로 제출하고, `wait-for` 화면 pattern은 동기화 전용이며 agent lifecycle은 오직 `list-agents` 상태 조회(`working|blocked|idle|done|unknown`)로만 판정합니다. 해당 조회가 증명되지 않으면 cmux는 `settled`를 절대 보고하지 않습니다. live token probe는 headless dispatch에 vendor call을 추가하지 않도록 `capabilities --worktree <path> --probe-live`와 각 subcommand 사용 시점에만 수행합니다. dispatch-core가 capability probe에 live 의도 신호를 넘겨주기 전까지 cmux live dispatch는 어떤 prompt 전달보다 먼저 `live_tui_capability_missing`으로 거부됩니다(세부 규칙은 플레이북 "cmux live sessions" 절).

```bash
NODE_OPTIONS= "$PRODUCT_HOME/scripts/agent-workflow.sh" dispatch \
  --orchestrator orca --runtime codex --role implementation \
  --execution-mode live-tui --issue 123 --worktree ../wt-123 \
  --prompt-file ../wt-123/.review/ISSUE-123-PROMPT.md
```
...
설정값은 transport `cmux|orca|herdr`, runtime `codex|claude|opencode|omp`, role `conductor|architect|implementation|reviewer|verifier|visual|release`만 허용하며, 위 우선순위에 따라 언제든 명시적으로 교체할 수 있습니다. `capabilities`로 선택 전에 세 adapter와 네 runtime을 함께 점검하고, live v4 dispatch receipt는 실제 선택된 transport·runtime·role·execution_mode를 기록합니다. Adapter 집합은 `scripts/lib/transport-registry.cjs`가 단일 원천으로 정의하며, CLI selection·shared core·routed admission이 모두 실행 시점에 이 registry를 읽고, receipt/telemetry schema의 hand-authored enum과의 정합성은 release gate의 parity 검사가 지킵니다(telemetry enum은 legacy `local` 값을 추가로 허용). 세 adapter는 공유 helper로 semver 파싱·floor 검사(`scripts/lib/semver.cjs`), capability JSON 방출(검증부 `scripts/lib/capability-result.cjs`의 field set을 참조하는 `scripts/lib/adapter-json.cjs`), help probe·runner 경로 가드·graceful 폴백·provenance 포맷(`scripts/lib/adapter-helpers.sh`)을 하나의 구현으로 공유합니다. CLI 인자 루프와 case dispatch는 adapter별로 유지됩니다.

`"$PRODUCT_HOME/scripts/agent-workflow.sh" capabilities --worktree ../wt-123`는 세 adapter와 네 runtime의 availability, version, role/mode 및 필요한 capability 근거를 JSON으로 보여 줍니다. Orca version은 `orca status --json`의 `result.runtime.appVersion`에서 읽으며, 값이 없거나 implausible하면 `unknown`입니다. cmux와 Orca도 실제 launch/list 계약을 admission 전에 증명하고, Herdr는 상속된 Herdr session(`HERDR_ENV=1` 및 비어 있지 않은 `HERDR_WORKSPACE_ID`/`HERDR_TAB_ID`/`HERDR_PANE_ID`)에서만 capability를 증명합니다. session context를 포함한 선택 capability probe 실패는 marker를 소비하지 않고 machine-readable reason으로 종료하며 다른 transport로 fallback하지 않습니다. Herdr receipt의 `external_handle`은 요청 label이나 pane ID가 아니라 반환된 workspace ID입니다. workspace liveness는 transport liveness일 뿐 workflow completion이 아니며, receipt는 confirmed command delivery가 아닌 launch intent/provenance를 기록합니다. OpenCode permission JSON은 top-level과 named primary `agent-workflow` agent 양쪽에서 `*`, `external_directory`, `webfetch`, `websearch`를 deny합니다. Read는 `edit`와 `bash`도 deny하고, write만 구현·빌드·테스트·Git 실행을 위해 양쪽을 명시적으로 allow합니다. Adapter는 검증한 내용을 `OPENCODE_CONFIG_CONTENT`로 전달하고 항상 `--agent agent-workflow`를 사용하므로 built-in/default agent fallback은 없습니다. Codex, Claude Code, OpenCode, OMP 모두 같은 public interface와 typed runtime adapter를 통해 conductor를 포함한 모든 role을 실행합니다.
...
새 runtime 공통 경로는 `agent-watchdog.sh`입니다. 각 시도는 `artifact_type:"agent_run"`과 runtime/role/version을 가진 RUN을 갱신하고, registry에 선언된 headless progress stream을 통해 등록된 runtime의 stdout 진행도 감시합니다. Codex direct read는 `runtimes/codex.sh`가, write/review는 `runtimes/codex-safe.sh`가 각각 registry의 `--json`을 적용하며, review의 `--output-last-message`는 canonical final JSON authority로 유지됩니다. Non-Codex REVIEWER가 거부되면 raw stdout도 non-authoritative `ISSUE-N-review-attempt<K>-output.log`로 보존하고 RUN의 `refusal_reason`으로 `unparseable_output` 등을 구분합니다. `attempt`는 watchdog 내부 시도 횟수이지 redispatch admission ordinal이나 실패 round가 아닙니다. `codex_run` 과거 산출물은 읽기 전용 호환 경로일 뿐 새 multi-runtime authority가 아니며, `codex-watchdog.sh`는 제거되었습니다 (#127).

dispatch의 실제 종료 상태를 기다리는 호출자는 append-only `ISSUE-N-EVENTS.jsonl`(watchdog은 `run_status`, admission 거부는 `admission_refused` 행 추가)을 읽는 `dispatch-core.sh await --issue N --cwd DIR [--timeout-seconds SECS]` 서브커맨드를 쓸 수 있습니다 (#163). RUN.json 계약과 기존 poll 루프의 의미는 그대로입니다.
...
설치하면 PRODUCT_HOME의 프로젝트 소유 `model-alloc.json`도 함께 생성됩니다. `"$PRODUCT_HOME/scripts/model-alloc.sh" --role implementation|reviewer --runner <codex|claude|opencode|omp>`은 실행 시 같은 schema로 설정을 검증하고, 증거가 없으면 안전한 기본 배치만 출력하며, canonical evidence의 연속된 findings round·작업량·계약 터치·재리뷰에 따라 배치 근거를 JSON으로 기록합니다. 구현 dispatch에 모델을 생략하면 PRODUCT_HOME의 이 설정을 사용해 선택 tuple을 결정하고 preflight와 launch에 동일하게 전달합니다. GPT-5.6의 공식 effort `none|low|medium|high|xhigh|max`는 세 모델 모두 사용할 수 있습니다. Evidence promotion은 더 높은 effort를 낮추지 않고 재리뷰는 reviewer effort를 한 단계만 낮춥니다. 리뷰 기본 우위는 source/release가 기록된 LiveBench의 `static_coding + reasoning` 단순 합으로 결정되고, 프로젝트가 명시적으로 완화할 때만 경고합니다. `agent-workflow.sh dispatch --orchestrator <cmux|orca|herdr> --allocate --allocator-role implementation`은 선택 runtime에 맞춰 구현 tuple을 자동 전달합니다. `implementation_by_runtime.<runtime>`이 설정되어 있으면 그 tuple을 쓰고, 없으면 기본 `implementation` tuple을 씁니다. 기본 implementation 모델(예: terra)의 `available_via`와 runtime이 일치하지 않으면 fail-closed되므로, opencode나 claude로 구현을 돌리려면 `implementation_by_runtime` 설정이 필요합니다. `implementation_by_runtime`이 설정된 runtime은 evidence adaptation에서 trivial_implementation 교체를 건너뛰고 effort만 조정합니다. REVIEWER는 `--produce-review --allocate --allocator-role reviewer`로 runtime별 tuple을 요청할 수 있습니다. 새 기본값은 Claude Code의 `sonnet` alias를 제공하지만, OpenCode 모델은 연결한 provider에 따라 `provider/model` 식별자가 다르므로 target이 `reviewer_by_runtime.opencode`에 preflightable tuple을 설정해야 합니다. OpenCode provider/model을 추측하거나 fallback하지 않습니다. 업그레이드는 사용자 설정 파일을 보존합니다. 이전 schema-v1 설정에 `available_via`가 없으면 upgrade는 경고만 내고 보존하며, dispatch는 알려진 모델 family의 runtime만 제한적으로 추론하고 알 수 없는 model은 fail-closed로 migration을 요구합니다.
...
REVIEWER는 `agent-workflow.sh dispatch --orchestrator <cmux|orca|herdr> --runtime <codex|claude|opencode|omp> --role reviewer --produce-review --model <model> --effort <effort>`로 실행합니다. 또는 `--allocate --allocator-role reviewer`로 runtime별 reviewer tuple을 명시적으로 요청할 수 있습니다. 선택 runtime은 capability-probed read mode를 제공해야 하며, Codex는 NDJSON stdout으로 progress/liveness를 제공하면서 `--output-last-message`의 exact JSON을 canonical review 입력으로 유지합니다. Non-Codex stdout의 prose-wrapped fenced JSON은 마지막 parseable block만 transcription 후보가 됩니다. Host-side는 그 뒤에도 JSON schema, producer, issue, live HEAD를 검증한 뒤 Git linked-worktree HEAD/ref lock 아래 canonical `.review/ISSUE-123-REVIEW.json`과 immutable snapshot을 원자 게시합니다. lock은 concurrent commit이 publication 사이에 끼어드는 것을 막고 실패 시 안전하게 해제됩니다. legacy `--read-only`는 liveness-only이며 canonical REVIEW publication을 대신하지 않습니다.
...
isolated worktree + env
        ↓
agent-workflow → selected runtime + cmux/Orca/Herdr adapter → watchdog → typed runtime adapter
        ↓
...
| `ISSUE-N-TRANSPORT.json` | v2 runtime provenance + transport/runner launch intent receipt; policy-opted canonical redispatch는 v3에 admission-bound route provenance를 파생 복사; v1은 legacy read-only 호환, 모두 confirmed delivery/completion의 증거가 아님 |
