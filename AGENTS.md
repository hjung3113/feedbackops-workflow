...
- `.review/schemas/` — JSON Schemas (draft-07) for workflow artifacts, including canonical ROUND-STATE and worker/review/verification evidence, with valid/invalid fixtures under `schemas/fixtures/`. List the live inventory with `ls .review/schemas/*.schema.json` and `ls .review/schemas/fixtures/`.
