...
1. **Scope** — resolve the issue/task, target repository, acceptance criteria, allowed touch set, and integration branch. Before locking scope for an exported-contract change, follow the playbook's pre-scope-lock impact pass; for repository-dependent Full Cluster ARCH decisions and test-matrix rows, apply its feasibility-evidence and test-matrix contracts. For a dispatched coding run, CONDUCTOR records the normative contract in `ISSUE-<n>-ROUND-STATE.json`; prompt amendments cannot override it.
...
5. **Gate implementation** — inspect commits, diffstat, changed files, blocker/run artifacts, and acceptance coverage. Do not trust the worker summary alone.
6. **Review** — Standard/Full changes get an independent clean-context review of the actual integrated diff. UI changes also follow the visual-reviewer persona.
7. **Verify** — run the target's verifier outside the Codex sandbox. If the target matches the bundled backend-pnpm/Vitest adapter, use `verify.sh` and require a current `ISSUE-<N>-VERIFY.json`. Otherwise use the target-native test gate and clearly report that the bundled canonical verifier adapter is not applicable; never fabricate its artifact.
8. **Close** — reconstruct state, re-review after fixes, present evidence to the user, and only then perform authorized merge/push/issue closure. Archive artifacts and close finished cmux workspaces.
