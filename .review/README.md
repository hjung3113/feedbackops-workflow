...
- `ISSUE-N-ROUND-STATE.json` — CONDUCTOR-owned canonical contract state. It replaces prose amendments and carries the current contract, AC manifest view, locked/open decisions, prior findings, commit range, live probes, and artifact pointers. Its top-level `revision` is the `manifest_revision` pinned by every dispatch; CODEX never edits it.
